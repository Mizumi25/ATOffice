...
import PixelOffice, { useOfficeTime, agentMgr, INTERACTABLES } from './components/PixelOffice'
...
function FileIconC({ name }) {
  ...
  const colors={jsx:'#61dafb',js:'#f7df1e',py:'#3776ab',md:'#4488cc',css:'#264de4',html:'#e34c26',json:'#888',sh:'#44aa44',ts:'#3178c6',tsx:'#3178c6',yml:'#cc8844',yaml:'#cc8844'}
      ...
      {open&&(
        <div style={{position:'absolute',bottom:70,right:10,zIndex:50,width:210,background:'var(--card)',borderRadius:16,border:'1.5px solid var(--glass-border)',boxShadow:'var(--neu-card)',overflow:'hidden'}}>
          ...
          <div style={{padding:14}}>
            <div style={{padding:'7px 10px',background:'var(--bg2)',borderRadius:8,marginBottom:10,fontSize:11,color:trackName?'var(--text)':'var(--text3)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
...
export default function App() {
  const {
    connected,agents,messages,tasks,leaderboard,stats,notifications,
    commandInput,chatInput,chatTarget,selectedAgent,activeModal,
    connectWS,setCommandInput,setChatInput,setChatTarget,setSelectedAgent,
    setActiveModal,sendCommand,sendChat,fetchStats,fetchTasks,continueWork,
    agentAction,addNotification
  } = useOfficeStore()
  ...
  const [wsEvents,setWsEvents]=useState([])   // ALL ws events — AgentActivityPanel reads these
  const [agentActivities,setAgentActivities]=useState({})
  const [agentStates,setAgentStates]=useState({})   // live tile positions for bubble tracking
  const [objectPopup,setObjectPopup]=useState(null)  // {id, msg} when player clicks object
  ...
  // Connect and poll
  useEffect(()=>{
    // Sync agentMgr positions every 150ms for speech bubble tracking
    const posIv = setInterval(()=>{
      try{ setAgentStates({...agentMgr.all()}) }catch{}
    },150)
    return ()=>clearInterval(posIv)
  },[])

  useEffect(()=>{
    connectWS();fetchStats();fetchTasks()
    const iv=setInterval(()=>{fetchStats();fetchTasks()},30000)
    return()=>clearInterval(iv)
  },[])

  // ── WS MESSAGE HANDLER — handles ALL event types including agent walks ──
  useEffect(()=>{
    const store=useOfficeStore.getState()
    const orig=store.handleWSMessage
    useOfficeStore.setState({handleWSMessage:(data)=>{

      // Terminal events → terminal panel
      // Feed ALL events to AgentActivityPanel (capped at 200)
      setWsEvents(ev=>[...ev.slice(-199),data])

      if(data.type==='terminal_event'){
        setTermEvents(ev=>[...ev.slice(-150),data]); return
      }
      // Activity labels → agent strip
      if(data.type==='agent_activity'){
        setAgentActivities(prev=>({...prev,[data.agent_id]:data.activity})); return
      }
      // File refresh → project file tree
      if(data.type==='refresh_files'){
        setRefreshTick(t=>t+1); return
      }
      // Task completed → switch to files tab
      if(data.type==='output_ready'){
        setRefreshTick(t=>t+1)
        setRightTab('files')
        addNotification(`${data.agent} finished: ${(data.title||'').slice(0,30)}`,'success')
      }

      // ── PATHFINDING EVENTS — trigger sprite movement ──────────────────

      // One agent walks to another (organic chat)
      if(data.type==='agent_walk'){
        if(data.from && data.to) agentMgr.walkToAgent(data.from, data.to)
        return
      }
      if(data.type==='agent_walk_object'){
        if(data.agent_id && data.object_key) agentMgr.walkToObject(data.agent_id, data.object_key)
        return
      }
      // Agents return to their desks after chatting
      if(data.type==='agent_return_home'){
        const ids = data.agents || []
        ids.forEach(id => agentMgr.returnHome(id))
        return
      }
      // Standup starts — everyone walks to PM table
      if(data.type==='standup_start'){
        Object.keys(store.agents || {})
          .filter(id => id !== 'pm')
          .forEach(id => agentMgr.walkToAgent(id, 'pm'))
        return
      }
      // Standup ends — everyone walks back home
      if(data.type==='standup_end'){
        Object.keys(store.agents || {}).forEach(id => agentMgr.returnHome(id))
        return
      }

      // All other events (messages, task updates, etc.) → default handler
      orig(data)
    }})
  },[agents])
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{
      if(Math.random()>0.5){
        const p={id:Date.now(),x:Math.random()*100,duration:6+Math.random()*6,rotation:Math.random()*360}
        setPetals(prev=>[...prev.slice(-8),p])
      }
    },3000)
    return()=>clearInterval(iv)
  },[])
  ...
  const handleObjectClick = useCallback((objId)=>{
    const obj = INTERACTABLES[objId]
    if(!obj) return
    const msgs = obj.playerMsg
    const msg = msgs[Math.floor(Math.random()*msgs.length)]
    setObjectPopup({id:objId, label:obj.label, msg})
    // Auto-dismiss after 4s
    setTimeout(()=>setObjectPopup(p=>p?.id===objId?null:p), 4000)
  },[])
    ...
    <div style={{width:'100vw',height:'100vh',display:'flex',flexDirection:'column',background:'var(--bg)',overflow:'hidden'}}>
      ...
      <div className="topbar">
        ...
        <div style={{display:'flex',alignItems:'center',gap:5}}>
          {connected
            ?<Wifi size={13} style={{color:'var(--working)'}}/>
            :<WifiOff size={13} style={{color:'var(--resting)'}}/>}
          <span style={{fontSize:11,color:connected?'var(--working)':'var(--resting)',fontWeight:500}}>
            {connected?'Online':'Offline'}
          </span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:4,color:'var(--text3)',fontSize:11}}>
          <TimeIcon hour={officeHour}/>
          <span>{String(Math.floor(officeHour)).padStart(2,'0')}:00</span>
        </div>
        {stats&&(
      ...
      <div className="main-layout" style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        <div className="office-area" ref={containerRef} style={{flex:1,position:'relative',overflow:'hidden',minHeight:200}}>
          <PixelOffice
            ...
            time={time} officeHour={officeHour}
            onObjectClick={handleObjectClick}
            onAgentClick={id=>{
              if(id==='whiteboard'){fetchTasks();setActiveModal('whiteboard')}
              else{setSelectedAgent(id);setActiveModal('agent')}
            }}
          ...
          <AgentDialogues
            ...
            containerW={canvasSize.w} containerH={canvasSize.h}
            agentActivities={agentActivities}
          />
          {petals.map(p=>(
            <div key={p.id} style={{
          ...
          {/* Object interaction popup */}
          {objectPopup&&(
            <div style={{
              position:'absolute',bottom:60,left:'50%',transform:'translateX(-50%)',
              zIndex:20,background:'rgba(248,244,240,0.97)',
              border:'2px solid #c8a868',borderRadius:14,
              padding:'10px 16px',maxWidth:260,textAlign:'center',
              boxShadow:'0 4px 20px rgba(0,0,0,0.2)',
              animation:'popIn 0.2s ease',
            }}>
              <div style={{fontSize:13,fontWeight:700,color:'#6a3a10',marginBottom:4}}>
                {objectPopup.label}
              </div>
              <div style={{fontSize:11,color:'#3a2010',lineHeight:1.55}}>
                {objectPopup.msg}
              </div>
              <button onClick={()=>setObjectPopup(null)}
                style={{marginTop:8,fontSize:10,background:'none',border:'1px solid #c8a868',
                  borderRadius:8,padding:'2px 10px',cursor:'pointer',color:'#8a5a20'}}>
                Close
              </button>
            </div>
          )}
        ...
        <div className="right-panel">
          <div className="panel-tabs">
            {[
              ['chat',     <MessageSquare size={11}/>, 'Chat'],
              ['files',    <FolderOpen size={11}/>,    'Files'],
              ['terminal', <Terminal size={11}/>, 'Activity'],
            ].map(([id,icon,label])=>(
          ...
          <div style={{flex:1,overflow:'hidden',minHeight:0,position:'relative'}}>
            {/* All 3 panels stay MOUNTED — display:none hides without unmounting */}
            {/* This prevents state loss when switching tabs */}
            <div style={{display:rightTab==='chat'?'flex':'none',flexDirection:'column',height:'100%',overflow:'hidden'}}>
              <ChatPanel
            ...
            <div style={{display:rightTab==='files'?'flex':'none',flexDirection:'column',height:'100%',overflow:'hidden'}}>
              <ProjectFileTree
            ...
            <div style={{display:rightTab==='terminal'?'flex':'none',flexDirection:'column',height:'100%',overflow:'hidden'}}>
              <AgentActivityPanel
                agents={agents}
                wsEvents={wsEvents}
                agentActivities={agentActivities}
      ...
      <div className="agent-strip">
        {agentArr.map(a=>(
          <div key={a.id} className={`agent-chip ${selectedAgent===a.id?'selected':''}`}
            onClick={()=>{setSelectedAgent(a.id);setActiveModal('agent')}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexShrink:0,paddingLeft:10,borderLeft:'1px solid var(--border)'}}>
          <input
            ...
            placeholder="Assign project to team... (Enter)"
            style={{width:220,fontSize:11}}
          />
          <button className="btn btn-primary btn-sm"
            onClick={()=>{if(commandInput.trim()){sendCommand(commandInput);setCommandInput('')}}}>
            <Send size={12}/>
          </button>
        </div>
      </div>
      ...
      {activeModal==='agent'&&selectedAgent&&agents[selectedAgent]&&(
        <AgentProfileModal
          ...
          onAction={action=>{agentAction(selectedAgent,action);setActiveModal(null)}}
        />
      )}
      ...
      <div className="modal" style={{width:420}}>
        <div className="modal-title">
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <Trophy size={16} style={{color:'var(--accent3)'}}/> Rankings
          </div>
          ...
          {leaderboard.map((a,i)=>(
            <div key={a.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 8px',borderRadius:10,background:i===0?'rgba(212,96,122,0.06)':'transparent',marginBottom:4}}>
              ...
              <div style={{textAlign:'right'}}>
                <div style={{color:'var(--green)',fontWeight:600}}>{a.productivity_points||0} pts</div>
                <div style={{fontSize:11,color:'var(--accent3)'}}>¥{(a.salary||0).toLocaleString()}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="modal-body">
          <div style={{display:'flex',gap:12,marginBottom:16,padding:12,background:'var(--bg2)',borderRadius:12,boxShadow:'var(--neu-in)'}}>
            ...
            <div>
              ...
              <div style={{color:'var(--text3)',fontSize:12}}>{agent.role}</div>
              <div style={{marginTop:6}}>
                <span className={`badge badge-${agent.status||'idle'}`}>
                  <div className={`status-dot ${agent.status||'idle'}`} style={{marginRight:4}}/>
                  {agent.status||'idle'}
                </span>
              </div>
            </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:16}}>
            {[
              ['Salary',`¥${(agent.salary||0).toLocaleString()}`,'var(--accent3)'],
              ['Points',`${agent.productivity_points||0}`,'var(--green)'],
              ['Status',agent.status||'idle','var(--blue)'],
            ].map(([l,v,c])=>(
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            ...
            {agent.status==='resting'
              ?<button className="btn btn-primary btn-sm" onClick={()=>onAction('resume')}><Play size={12}/>Wake Up</button>
              :<button className="btn btn-secondary btn-sm" onClick={()=>onAction('pause')}><Pause size={12}/>Break</button>
            }
          </div>
        </div>
      </div>
...
function WhiteboardModal({tasks,onClose,onRefresh}){
  ...
  const del=async id=>{
    setDeleting(id)
    try{await fetch(`${API}/tasks/${id}`,{method:'DELETE'});setLocal(p=>p.filter(t=>t.id!==id));onRefresh?.()}
    finally{setDeleting(null)}
  }
  const delAll=async status=>{
    await fetch(status?`${API}/tasks?status=${status}`:`${API}/tasks`,{method:'DELETE'})
    setLocal(p=>status?p.filter(t=>t.status!==status):p.filter(t=>t.status==='completed'))
    setConfirmAll(false);onRefresh?.()
  }
  ...
  return(
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{width:'90vw',maxWidth:760}}>
        <div className="modal-title">
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <ClipboardList size={15} style={{color:'var(--accent)'}}/> Task Board · {local.length}
          </div>
          <div style={{display:'flex',gap:6}}>
            {confirmAll?(
              <>
                <span style={{fontSize:11,color:'#e06060'}}>Delete {active}?</span>
                <button className="btn btn-danger btn-xs" onClick={()=>delAll()}>Yes</button>
                <button className="btn btn-ghost btn-xs" onClick={()=>setConfirmAll(false)}>No</button>
              </>
            ):active>0?(
              <button className="btn btn-danger btn-xs" onClick={()=>setConfirmAll(true)}>Clear Active</button>
            ):null}
            <button className="btn btn-ghost btn-xs" onClick={onClose}><X size={14}/></button>
          </div>
        </div>
    ...
    <div
      ...
      {canDel&&hover&&(
        <button onClick={e=>{e.stopPropagation();onDelete()}} disabled={isDeleting}
          style={{position:'absolute',top:4,right:4,background:'rgba(224,96,96,0.9)',border:'none',color:'#fff',borderRadius:5,width:17,height:17,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <X size={10}/>
        </button>
      )}
    </div>
...
function CommandModal({onClose,onSend}){
  ...
  const examples=[
    'Build a portfolio website with hero, about, contact sections',
    'Create a SaaS dashboard with auth, billing, and analytics',
    'Build a React Native mobile app for a food delivery service',
    'Create a REST API for a todo app with JWT auth and PostgreSQL',
    'Design a landing page for a Japanese tea shop with animations',
    'Build a real-time chat app with WebSockets and file uploads',
  ]
  return(
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{width:520}}>
        <div className="modal-title">
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <Users size={15} style={{color:'var(--accent)'}}/> Command the Team
          </div>
          <button className="btn btn-ghost btn-xs" onClick={onClose}><X size={14}/></button>
        </div>
        <div className="modal-body">
          <p style={{fontSize:12,color:'var(--text3)',marginBottom:14}}>
            19 specialists activated — PRD → Architecture → Design → Build → Test → Security → Deploy → Launch.
          </p>
          <textarea value={input} onChange={e=>setInput(e.target.value)}
            placeholder="Describe what to build..." rows={3} style={{marginBottom:14,resize:'vertical'}}/>
          <div style={{marginBottom:16}}>
            ...
            {examples.map((ex,i)=>(
              <div key={i} onClick={()=>setInput(ex)}
                style={{padding:'8px 12px',borderRadius:10,cursor:'pointer',fontSize:12,color:'var(--text2)',background:'var(--bg2)',marginBottom:5,border:'1px solid var(--border)',transition:'all 0.1s'}}
                onMouseOver={e=>e.currentTarget.style.borderColor='var(--pink2)'}
          ...
          <div style={{display:'flex',justifyContent:'flex-end',gap:8}}>
            ...
            <button className="btn btn-primary btn-sm" onClick={()=>input.trim()&&onSend(input)}>
              <Send size={12}/> Dispatch to 19 agents
            </button>
          </div>
        </div>
      </div>
